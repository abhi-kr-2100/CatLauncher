...
use crate::mods::paths::{get_mods_resource_path, get_stock_mods_dir, GetStockModsDirError};
...
pub async fn list_all_mods(
    ...
    let third_party_mods = list_all_third_party_mods(game_variant, resource_dir).await?;
    ...
    if let Some(release_version) = release_version {
        let stock_mods_dir =
            get_stock_mods_dir(game_variant, &release_version, data_dir, os).await?;
...
fn extract_stock_mod_from_modinfo(modinfo_content: &str) -> Result<StockMod, ExtractStockModError> {
    let entries: Vec<serde_json::Value> = serde_json::from_str(modinfo_content)?;

    // Find the MOD_INFO entry
    let mod_info = entries
        .iter()
        .find(|entry| entry.get("type").and_then(|t| t.as_str()) == Some("MOD_INFO"))
...
async fn list_all_stock_mods(stock_mods_dir: &Path) -> Result<Vec<Mod>, ListAllStockModsError> {
    let mut mods = Vec::new();
...
async fn list_all_third_party_mods(
    ...
    let mut mods = Vec::new();
    for (_, mod_data) in variant_mods {
        let third_party_mod = serde_json::from_value::<ThirdPartyMod>(mod_data.clone());
        match third_party_mod {
            Ok(third_party_mod) => mods.push(Mod::ThirdParty(third_party_mod)),
            Err(e) => return Err(ListThirdPartyModsError::ParseModsJson(e)),
        }
    }
