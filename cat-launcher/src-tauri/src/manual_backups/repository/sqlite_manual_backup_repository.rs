...
impl ManualBackupRepository for SqliteManualBackupRepository {
    ...
    async fn get_manual_backup_entry(&self, id: i64) -> Result<ManualBackupEntry, ManualBackupRepositoryError> {
        let pool = self.pool.clone();
    ...
    async fn delete_manual_backup_entry(&self, id: i64) -> Result<(), ManualBackupRepositoryError> {
        let pool = self.pool.clone();
        ...
        task::spawn_blocking(move || {
            ...
            conn.execute("DELETE FROM manual_backups WHERE id = ?1", rusqlite::params![id])
                .map_err(|e| ManualBackupRepositoryError::Delete(Box::new(e)))?;
            Ok(())
        })
        .await
