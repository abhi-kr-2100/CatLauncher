...
pub async fn run_updater(handle: AppHandle) {
  ...
  match updater.check().await {
    ...
    Ok(None) => {
      let _ =
        handle.emit("autoupdate-status", &UpdateStatus::UpToDate);
    }

    Ok(Some(update)) => {
      let _ =
        handle.emit("autoupdate-status", &UpdateStatus::Downloading);
      if let Err(e) = update
        .download_and_install(
          |_, _| {},
          || {
            let _ = handle
              .emit("autoupdate-status", &UpdateStatus::Installing);
          },
        )
        .await
      {
        ...
        let _ =
          handle.emit("autoupdate-status", &UpdateStatus::Success);
      }
    }
  }
}
